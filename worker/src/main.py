...
def _save_credentials(config: WorkerConfig, agent: WorkerAgent) -> None:
    ...
    if CONFIG_FILE.exists():
        ...
        # Remove old credential lines (including legacy BSNEXUS_REDIS_URL)
        lines = [
...
async def _run_loop(agent: WorkerAgent, config: WorkerConfig) -> None:
    """Core event loop: heartbeat + task polling."""
    executor = create_executor(config.executor_type)
    consumer = TaskConsumer(agent, executor)

    log.info("Worker %s listening for tasks (executor=%s, heartbeat=%ds, poll=%ds)",
             agent.worker_id, config.executor_type, config.heartbeat_interval, config.poll_interval)
...
async def run(config: WorkerConfig) -> None:
    ...
    agent.worker_id = config.worker_id
    agent.token = config.worker_token
    # Re-register to refresh auth token
    await agent.register()
